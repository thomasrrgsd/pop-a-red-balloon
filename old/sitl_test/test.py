...
# ASK IF TIS IS OK BEFORE FLYING
def condition_yaw_ccw_cw(heading, ccw_cw, relative=False):
    """
    ...
    msg = vehicle.message_factory.command_long_encode(
        ...
        ccw_cw,     # param 3, direction -1 ccw, 1 cw
...
def goto_position_target_local_ned(x_velocity):
    """
    Send SET_POSITION_TARGET_LOCAL_NED command to request the vehicle fly to a specified
    location in the x velocity frame.
    ...
    msg = vehicle.message_factory.set_position_target_local_ned_encode(
        ...
        0, 0,    # target system, target component
        mavutil.mavlink.MAV_FRAME_LOCAL_NED, # frame needs to be equal to 9 to have positions are relative to the vehicle’s current position and heading
        0b110111000111, # type_mask (only velocities enabled)
        0, 0, 0,
        x_velocity, 0, 0, # x, y, z velocity in m/s 
...
for i in range(1, 10):
  condition_yaw_ccw_cw(36, 1, True)
  time.sleep(2)
...
goto_position_target_local_ned(dist)
print("Going forward %d meters." % dist)
